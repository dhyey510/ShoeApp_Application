...
class DetailsFragment : Fragment(R.layout.fragment_detailspage), SizeOnClickInterface {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val productId = requireArguments().get("productId").toString()
                    ...
                    .addOnSuccessListener { querySnapshot ->
                        if(querySnapshot.isEmpty){
                            ...
                            for (item in querySnapshot) {
                                val qnty = item.toObject(ProductOrderModel::class.java).quantity
                                orderDatabaseReference.document(item.id).update("quantity", qnty!! + orderQuantity)
